...
const props = definePropst(['books'])
const booksRead = computed(() =>{
    return props.books.filter(book => book.isRead).length;
});

const booksReadMessage = computed(() =>{
    return booksRead.value >= props.books.length ? 'Parabéns, você leuu todos os livros' : `${booksRead.value} de ${props.books.length} livros lidos`;
});
